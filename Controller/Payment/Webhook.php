...
// use Getepay\Magento\Model\PaymentMethod;
...
/**
 * Webhook controller to handle Getepay order webhook
 *
 * ...
 */
class Webhook extends \Getepay\Getepe\Controller\BaseController
{
    ...
    public function __construct(
        ...
        $keyId                    = $this->config->getConfigData(Config::KEY_GETEPAY_KEY);
        $keySecret                = $this->config->getConfigData(Config::KEY_GETEPAY_IV);
    ...
    {
        echo 'Getepay Webhook processing started.'; exit;
        $this->logger->info("Getepay Webhook processing started.");
            ...
            if (!empty($razorpaySignature) === true)
            {
                ...
                if (isset($post['payload']['payment']['entity']['notes']['merchant_order_id']) === true)
                {
                    ...
                    if (empty($orderWebhookData['rzp_webhook_notified_at']) === true)
                    {
                        ...
                        $this->logger->info("Getepay Webhook: Updated WebhookNotifiedAt.");
                    ...
                    {
                        $this->logger->critical("Getepay Webhook: Webhook conflicts due to early execution for OrderID: " . $orderId);
        ...
        $this->logger->info("Getepay Webhook processing completed.");
    }
    ...
    {
        $this->logger->info("Getepay Webhook Event(" . $post['event'] . ") processing Started.");
        ...
        try
        {
            ...
            {
                $this->logger->info("Getepay Webhook: Quote ID not set for Getepay payment_id(:$paymentId)");
                return;
            }
        }
        catch (\Getepay\Api\Errors\Error $e)
        {
            $this->logger->critical("Getepay Webhook: fetching RZP order data(id:$rzpOrderId) "
        ...
        try
        {
            ...
            {
                $this->logger->info("Getepay inside order already processed with webhook orderID:" . $orderId
                ...
                if ($order)
                {
                    ...
                    )
                    {
                        ...
                        $this->logger->info("Getepay Webhook: "
                                            . " Event: " . $post['event']
                        ...
                        $order->addStatusHistoryComment(
                            __(
                                'Actual Amount %1 of %2, with Getepay Offer/Fee applied.',
                                "Authorized",
                    ...
                    {
                        $this->logger->info("Getepay Webhook: Sales Order and payment "
                            . "already exist for Getepay payment_id(:$paymentId)");
                        return;
                    }
                }
            }
        ...
        {
            $this->logger->critical("Getepay Webhook payment.authorized exeption, orderID:" . $orderId
                                    ." PaymentId: " . $paymentId
    ...
    protected function orderPaid(array $post)
    {
        ...
        if (isset($orderId) === false)
        {
            $this->logger->info("Getepay Webhook: Order ID not set for Getepay payment_id(:$paymentId)");
            return;
        }
            ...
            if (isset($salesOrder['entity_id']) && empty($salesOrder['entity_id']) === false)
            {
                ...
                if ($order)
                {
                    ...
                    )
                    {
                        ...
                        $order->addStatusHistoryComment(
                            __(
                                '%1 amount of %2 online, with Getepay Offer/Fee applied.',
                                "Captured",
                                $order->getBaseCurrency()->formatTxt($amountPaid)
                            )
                        );
        ...
        {
            $this->logger->critical("Getepay Webhook order.paid exeption, orderID:" . $orderId
                                    ." PaymentId: " . $paymentId
                                    ." and Message:" . $e->getMessage());
        }
    }
    ...
    public function getRzpOrder($orderId)
    {
        ...
        {
            $this->logger->critical("Getepay Webhook: fetching RZP order "
